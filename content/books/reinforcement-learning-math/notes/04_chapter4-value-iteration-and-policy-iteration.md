---
title: "第 4 章 值迭代与策略迭代（Value Iteration and Policy Iteration）— 从 4.1 开始"
...
补充对照例子见：[第 4 章补充：同一个例子对比值迭代与策略迭代](/books/reinforcement-learning-math/notes/04_supplement-value-iteration-vs-policy-iteration/)。
...
$$ v_k\to v^*,\qquad \pi_k\to \pi^* \quad (k\to\infty). $$
...
$$ v_k \xrightarrow{\text{greedy policy update}} \pi_{k+1} \xrightarrow{\text{one-step value update}} v_{k+1}. $$
...
$$ q_k(s,a) \doteq \underbrace{\sum_r p(r|s,a)r}_{\text{执行 }a\text{ 的平均即时奖励}} +\gamma \underbrace{\sum_{s'}p(s'|s,a)v_k(s')}_{\text{按 }v_k\text{ 估计下一状态未来值}}. $$
...
$$ v_{k+1}(s) = \sum_a \pi_{k+1}(a|s)q_k(s,a). $$
...
$$ r_{\mathrm{boundary}}=-1,\qquad r_{\mathrm{forbidden}}=-1,\qquad r_{\mathrm{target}}=1, $$

折扣率：

$$ \gamma=0.9. $$

![原书图 4.2(a)：2×2 网格，s2 为禁止区，s4 为目标区](/books/reinforcement-learning-math/images/1c44cec40e35189e55ae48728d4afce227ae1c81dedf33d39bd9c2015ab4c5d3.jpg)
![原书图 4.2(b)：第一次迭代得到的策略，其中 s1 因并列最大值选择了原地不动](/books/reinforcement-learning-math/images/9c1d61fe6c9a1727c3495f55e55a0c8543c3f51a045747bb548fdc8ff7f40481.jpg)
![原书图 4.2(c)：第二次迭代得到的最优策略](/books/reinforcement-learning-math/images/eaa563595e6b8647ffa36a0fcbb56404ac4a5051c9c185a12b9cf6c71dcee4fe.jpg)
...
$$ v_2(s_2)=1+\gamma\cdot1=1.9,\quad v_2(s_3)=1+\gamma\cdot1=1.9,\quad v_2(s_4)=1+\gamma\cdot1=1.9. $$
...
$$ v^*(s_4)=1+\gamma v^*(s_4) \quad\Rightarrow\quad v^*(s_4)=\frac{1}{1-\gamma}=10. $$
...
$$ \pi_k \xrightarrow{\text{policy evaluation}} v_{\pi_k} \xrightarrow{\text{policy improvement}} \pi_{k+1}. $$
...
$$ v_{\pi_k}^{(j+1)} = r_{\pi_k}+\gamma P_{\pi_k}v_{\pi_k}^{(j)}, \quad j=0,1,2,\dots \tag{4.4} $$
...
$$ \left\|v_{\pi_k}^{(j+1)}-v_{\pi_k}^{(j)}\right\|\lt \epsilon. $$
...
$$ \text{值迭代用 }v_k,\qquad \text{策略迭代用 }v_{\pi_k}. $$
...
> 如果
> $$
\gt  \pi_{k+1}=\arg\max_\pi(r_\pi+\gamma P_\pi v_{\pi_k}),
\gt  $$
> 那么
> $$
\gt  v_{\pi_{k+1}}\ge v_{\pi_k}.
\gt  $$
...
$$ \begin{aligned} v_{\pi_k}-v_{\pi_{k+1}} &= \left(r_{\pi_k}+\gamma P_{\pi_k}v_{\pi_k}\right) - \left(r_{\pi_{k+1}}+\gamma P_{\pi_{k+1}}v_{\pi_{k+1}}\right)\\ &\le \left(r_{\pi_{k+1}}+\gamma P_{\pi_{k+1}}v_{\pi_k}\right) - \left(r_{\pi_{k+1}}+\gamma P_{\pi_{k+1}}v_{\pi_{k+1}}\right)\\ &= \gamma P_{\pi_{k+1}}(v_{\pi_k}-v_{\pi_{k+1}}). \end{aligned} $$

读法：旧策略和新策略的价值差，最多等于“折扣后、按新策略转移一次的价值差”。

继续把这个不等式递推下去：

$$ v_{\pi_k}-v_{\pi_{k+1}} \le \gamma^n P_{\pi_{k+1}}^n (v_{\pi_k}-v_{\pi_{k+1}}). $$
...
$$ \text{值迭代：} v_k \rightarrow \pi_{k+1} \rightarrow v_{k+1} \quad(\text{只备份一步}); $$

$$ \text{策略迭代：} \pi_k \rightarrow v_{\pi_k} \rightarrow \pi_{k+1} \quad(\text{把当前策略评价完整}). $$
...
![原书图 4.3(a)：初始策略，两个状态都向左](/books/reinforcement-learning-math/images/bb3e4f481f105f7b594fe4d804eff1e84aff8e44e4912b22d876a7b83e1d4e46.jpg)
![原书图 4.3(b)：一次策略改进后得到的最优策略](/books/reinforcement-learning-math/images/0c6972d293df444c7e65e5e1d8c3e1c5109fe21e949172e22aa1ad2e5d64aa71.jpg)
...
$$ v_{\pi_0}(s_1) = -1+0.9v_{\pi_0}(s_1) $$

移项：

$$ 0.1v_{\pi_0}(s_1)=-1 \quad\Rightarrow\quad v_{\pi_0}(s_1)=-10. $$
...
![原书图 4.4(a-策略)：随机初始策略](/books/reinforcement-learning-math/images/66670c314f81b3a915d68baee1f63ee3964d7b38c6b7eb0d3f8c7ebc95d89484.jpg)
![原书图 4.4(h-策略)：收敛后的最优策略](/books/reinforcement-learning-math/images/0216770b2280acfe40759b53593a8c497861bdf696bfc55535736f6848651d55.jpg)
![原书图 4.4(h-价值)：收敛后的状态值分布](/books/reinforcement-learning-math/images/f7e4d0d21c0865709b0995c46f69f036881150eea89c2dd342ef9531a5db9bfc.jpg)
...
原因是折扣因子 $\gamma\lt 1$。距离目标越远，拿到正奖励之前要走的步数越多，奖励被折扣得越厉害。
...
$\|v_{k+1}-v_k\|\lt \epsilon$，因为 $v_k$ 只是中间估计，策略短暂不变不一定代表价值已经收敛。
...
$$ \pi_0 \xrightarrow{\text{PE}} v_{\pi_0} \xrightarrow{\text{PI}} \pi_1 \xrightarrow{\text{PE}} v_{\pi_1} \xrightarrow{\text{PI}} \pi_2 \xrightarrow{\text{PE}}\cdots $$
...
$$ v_0 \xrightarrow{\text{PU}} \pi_1' \xrightarrow{\text{VU}} v_1 \xrightarrow{\text{PU}} \pi_2' \xrightarrow{\text{VU}} v_2 \xrightarrow{\text{PU}}\cdots $$
...
$$ \pi_1 = \arg\max_\pi(r_\pi+\gamma P_\pi v_{\pi_0}) = \arg\max_\pi(r_\pi+\gamma P_\pi v_0). $$
...
![原书图 4.5：值迭代、截断策略迭代和策略迭代的关系](/books/reinforcement-learning-math/images/92a8fcc0d8160cacb4c932fe53b160a56b510b6ace694cc362bc69e69011ea0f.jpg)
...
3. 对 $j=0,1,\ldots,j_{\mathrm{truncated}}-1$，对每个状态 $s$ 做：
   ...
   $$ v_k^{(j+1)}(s) = \sum_a\pi_k(a|s) \left[ \underbrace{\sum_r p(r|s,a)r}_{\text{平均即时奖励}} + \gamma \underbrace{\sum_{s'}p(s'|s,a)v_k^{(j)}(s')}_{\text{用上一轮近似值估计未来}} \right]. $$
...
$$ v_\pi(s_2)=\frac{1}{1-0.9}=10, \qquad v_\pi(s_1)=0.9\times 10=9. $$
...
$$ v_{\pi_k}^{(j+1)} = r_{\pi_k} + \gamma P_{\pi_k}v_{\pi_k}^{(j)}, \qquad j=0,1,2,\ldots $$
...
$$ v_{\pi_k}^{(j+1)}\ge v_{\pi_k}^{(j)}, \qquad j=0,1,2,\ldots $$
...
$$ \begin{aligned} v_{\pi_k}^{(j+1)}-v_{\pi_k}^{(j)} &= \left(r_{\pi_k}+\gamma P_{\pi_k}v_{\pi_k}^{(j)}\right) - \left(r_{\pi_k}+\gamma P_{\pi_k}v_{\pi_k}^{(j-1)}\right)\\ &= \gamma P_{\pi_k} \left(v_{\pi_k}^{(j)}-v_{\pi_k}^{(j-1)}\right). \end{aligned} $$

读法：即时奖励 $r_{\pi_k}$ 抵消了，只剩“上一次价值差”经过转移矩阵 $P_{\pi_k}$ 和折扣 $\gamma$ 传播。

继续展开：

$$ v_{\pi_k}^{(j+1)}-v_{\pi_k}^{(j)} = \gamma^jP_{\pi_k}^j \left(v_{\pi_k}^{(1)}-v_{\pi_k}^{(0)}\right). \tag{4.5} $$
...
$$ \begin{aligned} v_{\pi_k}^{(1)} &= r_{\pi_k} + \gamma P_{\pi_k}v_{\pi_k}^{(0)}\\ &= r_{\pi_k} + \gamma P_{\pi_k}v_{\pi_{k-1}}\\ &\ge r_{\pi_{k-1}} + \gamma P_{\pi_{k-1}}v_{\pi_{k-1}}\\ &= v_{\pi_{k-1}}\\ &= v_{\pi_k}^{(0)}. \end{aligned} $$

不等号来自策略改进：

$$ \pi_k = \arg\max_\pi \left(r_\pi+\gamma P_\pi v_{\pi_{k-1}}\right). $$
...
$$ j_{\mathrm{truncated}}=1 \Rightarrow \text{value iteration}, \qquad 1\lt j_{\mathrm{truncated}}\lt \infty \Rightarrow \text{truncated policy iteration}, \qquad j_{\mathrm{truncated}}=\infty \Rightarrow \text{policy iteration}. $$
...
$$ \boxed{ \text{每轮迭代都在更新价值，也在更新策略} } $$

只是三种算法把这两件事的轻重安排得不同。

**值迭代**里，策略藏在 $\max/\arg\max$ 里面：

$$ \pi_{k+1}\in\arg\max_\pi(r_\pi+\gamma P_\pi v_k), \qquad v_{k+1}=r_{\pi_{k+1}}+\gamma P_{\pi_{k+1}}v_k. $$
...
$$ v_k^{(j+1)} = r_{\pi_{k+1}}+\gamma P_{\pi_{k+1}}v_k^{(j)}, \qquad j=0,1,\ldots,j_{\mathrm{truncated}}-1. $$
...
$$ \text{用当前价值改策略} \quad\Longleftrightarrow\quad \text{用当前策略改价值}. $$
...
$$ \boxed{ \text{VI、PI、TPI 都是在做价值和策略的相互更新，只是策略评价做得深浅不同。} } $$
...
$$ v_0 \xrightarrow{T^*} v_1 \xrightarrow{T^*} v_2 \xrightarrow{T^*} v_3 \xrightarrow{T^*} \cdots, $$
...
[第 4 章补充：同一个例子对比值迭代与策略迭代](/books/reinforcement-learning-math/notes/04_supplement-value-iteration-vs-policy-iteration/)。
