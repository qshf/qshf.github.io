---
title: "第 5 章 蒙特卡洛方法（Monte Carlo Methods）- 5.1-5.7"
...
$$ \mathbb {E} [ X ] = \sum_{x\in\mathcal X} \underbrace{p(x)}_{\text{这个取值出现的概率}} \underbrace{x}_{\text{这个取值本身}}. $$
...
$$ \bar{x} = \frac{1}{n} \underbrace{(x_1+x_2+\cdots+x_n)}_{\text{把采到的结果加起来}}. $$
...
$$ X= \begin{cases} 1, & \text{正面},\\ -1, & \text{反面}. \end{cases} $$
...
![原书图 5.2：抛硬币样本均值逐渐接近期望](/books/reinforcement-learning-math/images/574909b5dcc25354a90c3fe239a0350c6e537732a936c29f6fdd3dc4554fc555.jpg)
...
$$ \mathbb E[\bar x] = \frac{1}{n}\sum_{i=1}^{n}\mathbb E[X] = \frac{1}{n}\cdot n\mathbb E[X] = \mathbb E[X]. $$
...
$$ \operatorname{var}(\bar x) = \frac{1}{n^2}\sum_{i=1}^{n}\operatorname{var}(X) = \frac{1}{n^2}\cdot n\operatorname{var}(X) = \frac{1}{n}\operatorname{var}(X). $$
...
$$ \operatorname{var}(X) =\mathbb E[X^2]-\mathbb E[X]^2 =1-0=1. $$
...
$$ q_\pi(s,a) =\mathbb E[G_t|S_t=s,A_t=a] \approx \frac{1}{n}\sum_{i=1}^{n}g_\pi^{(i)}(s,a). $$
...
$$ \begin{array}{l} \pi_{k+1}(s) = \arg\max_\pi \sum_a \pi(a|s) \left[ \sum_r p(r|s,a)r +\gamma\sum_{s'}p(s'|s,a)v_{\pi_k}(s') \right] \\ = \arg\max_\pi \sum_a \pi(a|s)q_{\pi_k}(s,a), \quad s\in\mathcal S. \end{array} $$
...
$$ q_{\pi_k}(s,a) = \underbrace{\sum_r p(r|s,a)r}_{\text{动作 }a\text{ 的期望即时奖励}} + \gamma \underbrace{\sum_{s'}p(s'|s,a)v_{\pi_k}(s')}_{\text{下一状态价值的期望}}. $$
...
$$ \begin{array}{l} q_{\pi_k}(s,a) = \mathbb E[G_t|S_t=s,A_t=a] \\ = \mathbb E[ R_{t+1}+\gamma R_{t+2}+\gamma^2R_{t+3}+\cdots \mid S_t=s,A_t=a]. \end{array} $$
...
$$ \frac{1}{n}\sum_{i=1}^{n}g_{\pi_k}^{(i)}(s,a) = \frac{ \underbrace{g_{\pi_k}^{(1)}(s,a)+\cdots+g_{\pi_k}^{(n)}(s,a)}_{\text{从同一状态-动作对出发得到的多次回报}} }{n}. $$
...
$$ q_k(s,a) \approx q_{\pi_k}(s,a) = \mathbb E[G_t|S_t=s,A_t=a], $$
...
![原书图 5.3：MC Basic 的 3x3 网格例子](/books/reinforcement-learning-math/images/0f8e53e9dad4e83c5ddd42f70e6c4a987fe6c2241502c884f8a06754afd763a9.jpg)
> **原书图 5.3**：黄色格是 forbidden states，青色格 $s_9$ 是 target state；箭头表示初始策略 $\pi_0$。本例用它展示从 $s_1$ 出发时，怎样比较五个动作的动作值。

奖励设置为：

$$ r_{\mathrm{boundary}}=r_{\mathrm{forbidden}}=-1,\qquad r_{\mathrm{target}}=1,\qquad \gamma=0.9. $$
...
$$ r_{\mathrm{boundary}}=-1,\qquad r_{\mathrm{forbidden}}=-10,\qquad r_{\mathrm{target}}=1,\qquad \gamma=0.9. $$

![原书图 5.4(a)：episode length 等于 1 时的最终价值和策略](/books/reinforcement-learning-math/images/da8bdefcbe719f1b1d4ad98cad7fdc1108b5e2ca18a2a00e1e6c1839a46d002a.jpg)
> **原书图 5.4(a)**：episode length 只有 1 时，只有目标附近状态能很快获得非零正奖励；更远的状态看不到目标，价值估计多为 0。

![原书图 5.4(h)：episode length 等于 100 时的最终价值](/books/reinforcement-learning-math/images/ad3b96baab135111b650e3dbaf4ca3cd9c40352f8215f759fba6e110cd93e117.jpg)
...
$$ G_{\text{len}=5} = \gamma^3+\gamma^4 = 0.729+0.6561 = 1.3851. $$
...
$$ \text{到不了目标} \Rightarrow \text{episode 里没有正奖励} \Rightarrow G_t \text{ 很可能为 }0 \Rightarrow q_k(s,a)\text{ 难以区分动作好坏}. $$
...
$$ q_\pi(s,a) \approx \frac{1}{n}\sum_{i=1}^{n}g_\pi^{(i)}(s,a). $$
...
$$ \begin{array}{l} s_1 \xrightarrow{a_2} s_2 \xrightarrow{a_4} s_1 \xrightarrow{a_2} s_2 \xrightarrow{a_3} s_5 \xrightarrow{a_1}\cdots \quad [\text{original episode}]\\ s_2 \xrightarrow{a_4} s_1 \xrightarrow{a_2} s_2 \xrightarrow{a_3} s_5 \xrightarrow{a_1}\cdots \quad [\text{subepisode from }(s_2,a_4)]\\ s_1 \xrightarrow{a_2} s_2 \xrightarrow{a_3} s_5 \xrightarrow{a_1}\cdots \quad [\text{subepisode from }(s_1,a_2)]\\ s_2 \xrightarrow{a_3} s_5 \xrightarrow{a_1}\cdots \quad [\text{subepisode from }(s_2,a_3)]\\ s_5 \xrightarrow{a_1}\cdots \quad [\text{subepisode from }(s_5,a_1)]. \end{array} $$
...
$$ s_A \xrightarrow{a_R} s_B \xrightarrow{a_U} s_C \xrightarrow{a_L} s_D, $$
...
$$ A \xrightarrow{\mathrm{loop},\,r=0} A \xrightarrow{\mathrm{loop},\,r=1} B \xrightarrow{\mathrm{finish},\,r=3} T $$

里，$(A,\mathrm{loop})$ 出现了两次。第二次出现后的后缀轨迹是：

$$ A \xrightarrow{\mathrm{loop},\,r=1} B \xrightarrow{\mathrm{finish},\,r=3} T, $$
...
$$ \mathrm{Returns}(s,a)=0,\qquad \mathrm{Num}(s,a)=0, $$
...
$$ q(s_t,a_t) \leftarrow \frac{\mathrm{Returns}(s_t,a_t)} {\mathrm{Num}(s_t,a_t)}. $$

5. 对当前状态 $s_t$ 做贪心策略改进：

$$ \pi(a|s_t) = \begin{cases} 1, & a=\arg\max_a q(s_t,a),\\ 0, & \text{otherwise}. \end{cases} $$
...
$$ q(A,\mathrm{loop})=0,\qquad q(A,\mathrm{exit})=0,\qquad q(B,\mathrm{finish})=0. $$
...
$$ g\leftarrow \gamma g+r_3 =0.9\cdot 0+3 =3. $$
...
$$ \mathrm{Returns}(B,\mathrm{finish})=3,\qquad \mathrm{Num}(B,\mathrm{finish})=1, $$

$$ q(B,\mathrm{finish}) = \frac{3}{1} =3. $$

再往前一步 $t=1$。这是第二次访问 $(A,\mathrm{loop})$：

$$ g\leftarrow \gamma g+r_2 =0.9\cdot 3+1 =3.7. $$
...
$$ \mathrm{Returns}(A,\mathrm{loop})=3.7,\qquad \mathrm{Num}(A,\mathrm{loop})=1, $$

$$ q(A,\mathrm{loop}) = \frac{3.7}{1} =3.7. $$

再往前一步 $t=0$。这是第一次访问 $(A,\mathrm{loop})$：

$$ g\leftarrow \gamma g+r_1 =0.9\cdot 3.7+0 =3.33. $$
...
$$ \mathrm{Returns}(A,\mathrm{loop}) \leftarrow 3.7+3.33 =7.03, $$

$$ \mathrm{Num}(A,\mathrm{loop}) \leftarrow 1+1 =2. $$

于是样本平均动作值变成：

$$ q(A,\mathrm{loop}) = \frac{7.03}{2} =3.515. $$
...
$$ q(A,\mathrm{loop})=3.515\gt q(A,\mathrm{exit})=0, $$
...
$$ \mathrm{Returns}(A,\mathrm{exit})=1,\qquad \mathrm{Num}(A,\mathrm{exit})=1, $$

$$ q(A,\mathrm{exit}) = \frac{1}{1} =1. $$
...
$$ q(A,\mathrm{loop})=3.515\gt q(A,\mathrm{exit})=1, $$

所以策略仍然是：

$$ \pi(A)=\mathrm{loop},\qquad \pi(B)=\mathrm{finish}. $$

这个结果符合直觉：在 $A$ 直接退出只能拿到 $1$；而当前样本显示 $\mathrm{loop}$ 后续可能到达 $B$ 并 finish，得到更高回报。注意这里的数值 $3.515$ 是从同一条 episode 里的两个 visit 平均出来的：

$$ \frac{G_1+G_0}{2} = \frac{3.7+3.33}{2} =3.515. $$

所以 $\mathrm{loop}$ 比 $\mathrm{exit}$ 更好。

这个例子展示了 MC Exploring Starts 的完整工作流：

$$ \text{选探索性起点} \rightarrow \text{生成 episode} \rightarrow \text{从后往前算每个 visit 的 return} \rightarrow \text{更新 }q(s,a) \rightarrow \text{立刻贪心改进策略}. $$
...
如果策略只按当前估计贪心，它会一直选 $a_1$，因为 $1.0\gt 0.0$。但真实最优动作其实是 $a_2$。这就是 insufficient exploration 探索不足 的危险。
...
$$ \text{一条 episode 只用起点} \quad\Longrightarrow\quad \text{一条 episode 的每次 visit 都能用}; $$

$$ \text{攒齐很多 episode 再更新} \quad\Longrightarrow\quad \text{每条 episode 后就更新}. $$
...
$$ \pi(a|s) = \begin{cases} 1-\frac{\epsilon}{|\mathcal A(s)|}\left(|\mathcal A(s)|-1\right), & \text{for the greedy action},\\ \frac{\epsilon}{|\mathcal A(s)|}, & \text{for the other }|\mathcal A(s)|-1\text{ actions}. \end{cases} $$

贪心动作的概率也可以改写成：

$$ 1-\frac{\epsilon}{|\mathcal A(s)|}\left(|\mathcal A(s)|-1\right) = 1-\epsilon+\frac{\epsilon}{|\mathcal A(s)|}. $$
...
$$ 1-\epsilon+\frac{\epsilon}{|\mathcal A(s)|} \ge \frac{\epsilon}{|\mathcal A(s)|}, $$
...
$$ \mathcal A(s)=\{a_1,a_2,a_3\}, \qquad |\mathcal A(s)|=3. $$
...
$$ \frac{\epsilon}{|\mathcal A(s)|} = \frac{0.3}{3} =0.1. $$

贪心动作概率为：

$$ 1-\epsilon+\frac{\epsilon}{|\mathcal A(s)|} = 1-0.3+0.1 =0.8. $$
...
3. 如果 $x\lt \epsilon$，从所有动作 $\mathcal A(s)$ 中均匀随机选一个动作；注意这里仍可能再次选到贪心动作。

为什么这样得到的概率和上面一致？

贪心动作有两条路径被选中：

$$ \Pr(\text{greedy}) = \Pr(x\ge\epsilon) + \Pr(x\lt \epsilon)\Pr(\text{random chooses greedy}). $$

代入：

$$ \Pr(\text{greedy}) = (1-\epsilon)+\epsilon\cdot\frac{1}{|\mathcal A(s)|} = 1-\epsilon+\frac{\epsilon}{|\mathcal A(s)|}. $$

任意非贪心动作只有探索阶段能被选中：

$$ \Pr(\text{non-greedy action}) = \epsilon\cdot\frac{1}{|\mathcal A(s)|} = \frac{\epsilon}{|\mathcal A(s)|}. $$
...
$$ 1-\frac{|\mathcal A(s)|-1}{|\mathcal A(s)|}\epsilon = 1-\epsilon+\frac{\epsilon}{|\mathcal A(s)|}. $$
...
$$ \text{greedy improvement} \quad\Longrightarrow\quad \epsilon\text{-greedy improvement}. $$
...
No 的意思是：$\Pi_\epsilon$ 只是所有策略 $\Pi$ 的一个子集。只要 $\epsilon\gt 0$，策略就必须保留一些探索概率，不能把最优动作概率设成 1。因此它可能不是整个 $\Pi$ 中真正的 optimal policy 最优策略。

不过，如果 $\epsilon$ 很小，$\Pi_\epsilon$ 中的最优策略会很接近真正的最优策略。比如两个动作时，$\epsilon=0.02$，贪心动作概率是：

$$ 1-\epsilon+\frac{\epsilon}{2} = 1-0.02+0.01 =0.99. $$
...
![原书图 5.5(a)：MC ε-Greedy 的初始均匀策略](/books/reinforcement-learning-math/images/dc2c6ade03a715cebaa12302aab76346bc85ae04b989bc305d2d010b76d5de76.jpg)
> **原书图 5.5(a)**：初始策略对每个动作给相同概率，因此每个格子里各方向都一样粗。

![原书图 5.5(b)：第一次迭代后的 ε-贪心策略](/books/reinforcement-learning-math/images/577663754974dea0650b1479fa71e72877b2c1133d371d4f4d3b08668750b072.jpg)
> **原书图 5.5(b)**：经过一条超长 episode 后，许多状态已经根据估计出的动作值偏向某个更好的方向，但仍保留探索概率。

![原书图 5.5(c)：第二次迭代后的 ε-贪心策略](/books/reinforcement-learning-math/images/321e5657d53437cd84e169960bca23774ea090959289a76d6ec35c103f17143a.jpg)
...
$$ \text{exploring starts} \quad\Longrightarrow\quad \text{soft policy / }\epsilon\text{-greedy policy}. $$
...
$$ \pi(a|s) = \begin{cases} 1-\epsilon+\frac{\epsilon}{|\mathcal A(s)|}, & a=a^*,\\ \frac{\epsilon}{|\mathcal A(s)|}, & a\ne a^*. \end{cases} $$
...
![原书图 5.6(a)：ε=0 时的策略价值](/books/reinforcement-learning-math/images/eba1d73fdd6a5a1da4cf19ab2392bc5a272ccac538560a7d237e5699b3144158.jpg)
> **原书图 5.6(a)**：$\epsilon=0$ 时是 greedy policy，目标和目标附近状态价值很高。

![原书图 5.6(d)：ε=0.5 时的策略价值](/books/reinforcement-learning-math/images/4a9b7844e35ae8ef53866afc4c8363a1bd019fc3a01f41b7a425733c3e3e78ae.jpg)
...
$$ \epsilon=0:\quad \text{留在目标附近，持续拿正奖励;} $$

$$ \epsilon\text{ 大}:\quad \text{留在目标附近时随机动作太多，容易进入 forbidden areas;} $$
...
![原书图 5.8(d)：ε=1 时一百万步内各状态-动作访问次数](/books/reinforcement-learning-math/images/81e3b9ee3e5be117e7da9421f940b56d86c9d58776f44bb0bbbf03a2135c43db.jpg)
...
![原书图 5.8(h)：ε=0.5 时一百万步内各状态-动作访问次数](/books/reinforcement-learning-math/images/b07e8f38f64b54024c170b98600db03e3e15cd1073afc057b68ca5ff6eccb23f.jpg)
> **原书图 5.8(h)**：$\epsilon=0.5$ 时仍能探索所有动作，但访问次数高度不均匀；高概率动作获得大量样本，低概率动作样本很少。

所以：

$$ \epsilon\uparrow \Rightarrow \text{exploration ability 更强，访问更均匀;} $$

$$ \epsilon\downarrow \Rightarrow \text{exploitation 更强，访问更集中。} $$
...
$$ \epsilon_0\text{ 较大} \quad\Longrightarrow\quad \epsilon_k\text{ 逐渐减小}. $$
...
$$ \epsilon\text{ 大} \Rightarrow \text{探索强，但最优性差;} $$

$$ \epsilon\text{ 小} \Rightarrow \text{利用强，更接近最优策略，但探索弱。} $$
...
$$ \text{Policy iteration} \xrightarrow{\text{用 MC 估计替代模型计算}} \text{MC Basic} \xrightarrow{\text{更充分利用 visits}} \text{MC Exploring Starts} \xrightarrow{\epsilon\text{-greedy soft policy}} \text{MC }\epsilon\text{-Greedy}. $$

本章最后引入 exploration-exploitation tradeoff 探索-利用权衡：

$$ \epsilon\uparrow \Rightarrow \text{exploration 更强，但 exploitation/optimality 更弱}; $$

$$ \epsilon\downarrow \Rightarrow \text{exploitation 更强，但 exploration 更弱}. $$
...
$$ \text{没有充分探索} \Rightarrow \text{动作值估不准} \Rightarrow \text{策略改进可能选错动作}. $$

### Q7：如何避免 exploring starts？

基本思路是让策略变成 soft policy 软策略：

$$ \pi(a|s)\gt 0,\qquad \forall s,a. $$
...
No：只要 $\epsilon\gt 0$，策略必须给非贪心动作留概率，因此它通常不是整个策略集合 $\Pi$ 中的 pure optimal policy 纯最优策略。
...
$$ \text{MC Basic} \rightarrow \text{MC Exploring Starts} \rightarrow \text{MC }\epsilon\text{-Greedy}. $$
...
$$ \text{核心思想简单} \quad\rightarrow\quad \text{为了样本效率和现实可用性，逐步增加机制}. $$
...
$$ \text{价值估计} = \text{回报的均值估计} = \text{用很多次采样回报做平均}; \qquad \text{策略改进} = \text{对估计出的动作值贪心}; \qquad \text{样本效率} = \text{尽量利用每条轨迹里的每一次访问}; \qquad \text{持续探索} = \text{让每个动作始终保留正概率}; \qquad \epsilon = \text{探索-利用权衡的旋钮}. $$
