---
title: "第 6 章 随机近似（Stochastic Approximation）- 6.1-6.4"
...
$$ v_\pi(s)=\mathbb E_\pi[G_t|S_t=s], \qquad q_\pi(s,a)=\mathbb E_\pi[G_t|S_t=s,A_t=a]. $$
...
补充结构梳理见：[第 6 章补充：6.1-6.3 结构梳理](/books/reinforcement-learning-math/notes/06_supplement-6-1-6-3-structure/)。
...
$$ \bar x = \frac{1}{n} \underbrace{(x_1+x_2+\cdots+x_n)}_{\text{全部样本加起来}}. $$
...
$$ w _ {k + 1} = \frac {1}{k} \sum_ {i = 1} ^ {k} x _ {i}. $$

把最后一个样本 $x_k$ 单独拆出来：

$$ w _ {k + 1} = \frac {1}{k} \left(\sum_ {i = 1} ^ {k - 1} x _ {i} + x _ {k}\right). $$
...
$$ w _ {k + 1} = \frac {1}{k} \left((k - 1) w _ {k} + x _ {k}\right). $$

展开：

$$ w_{k+1} = \frac{k-1}{k}w_k+\frac{1}{k}x_k. $$

把 $\frac{k-1}{k}w_k$ 写成 $w_k-\frac{1}{k}w_k$：

$$ w_{k+1} = w_k-\frac{1}{k}w_k+\frac{1}{k}x_k. $$
...
$$ \text{new estimate} = \text{old estimate} - \text{stepsize}\times \text{current error}. $$

其中

$$ \underbrace{w_k-x_k}_{\text{当前估计相对新样本的误差}} $$

如果 $w_k\gt x_k$，更新会把 $w$ 往下拉；如果 $w_k\lt x_k$，更新会把 $w$ 往上推。
...
读法：当前估计 $w_k$ 朝新样本 $x_k$ 移动，移动比例由 $\alpha_k\gt 0$ 控制。
...
$$ \text{new value} = \text{old value} + \alpha \left( \text{target} - \text{old value} \right). $$

也就是：

$$ \text{更新} = \text{旧估计} + \text{步长} \times \text{误差}. $$
...
$$ \text{新估计} = \text{旧估计} - \text{步长} \times \text{当前观测到的误差}. $$
...
$$ 2(w-3)=0 \quad\Longrightarrow\quad w^*=3. $$
...
$$ \underbrace{\tilde g(w,\eta)}_{\text{我们实际看到的输出}} = \underbrace{g(w)}_{\text{真实函数值}} + \underbrace{\eta}_{\text{观测误差/噪声}}. $$

读法：输入 $w$ 后，系统内部真实地产生 $g(w)$，但我们看到的结果被噪声 $\eta$ 污染了。

![原书图 6.2：只能观察到带噪声的黑盒输出](/books/reinforcement-learning-math/images/a07547d2841e0afacc2aa9c712d76094f5978ea5767c9455f9f17aa466740322.jpg)
...
$$ \underbrace{w_{k+1}}_{\text{下一次估计}} = \underbrace{w_k}_{\text{当前估计}} - \underbrace{a_k}_{\text{步长}} \underbrace{\tilde g(w_k,\eta_k)}_{\text{当前带噪声观测}}. $$
...
- 如果 $w_k\gt w^*$，通常 $g(w_k)\gt 0$，更新会让 $w_k$ 变小，往根靠近；
- 如果 $w_k\lt w^*$，通常 $g(w_k)\lt 0$，更新会让 $w_k$ 变大，也往根靠近。
...
![原书图 6.3(a)：RM 估计值在噪声下靠近真根](/books/reinforcement-learning-math/images/a4d23a6e4b95b594d6d21212ffb2a5c0c85e409d1cec1048132868d44c8c927a.jpg)

![原书图 6.3(b)：每一步的观测噪声](/books/reinforcement-learning-math/images/09f82474ae2f719f93d0f4786de3bb56e1db761f3aa175e8fe18075cd2bd7df7.jpg)
...
$$ w_{k+1} = w_k-a_k\left(g(w_k)+\eta_k\right). $$

Figure 6.3 的例子取：

$$ g(w)=w^3-5, \qquad a_k=\frac1k. $$
...
$$ \begin{aligned} w_{k+1} &= w_k-\frac1k\left(g(w_k)+\eta_k\right)\\ &= w_k-\frac1k\left(w_k^3-5+\eta_k\right). \end{aligned} $$

因此这个例子的具体更新式是：

$$ w_{k+1} = w_k-\frac1k\left(w_k^3-5+\eta_k\right), \qquad w_1=0. $$

如果只写前 3 次迭代，就是：

$$ w_2 = w_1-\left(w_1^3-5+\eta_1\right) = 5-\eta_1, $$

$$ w_3 = w_2-\frac12\left(w_2^3-5+\eta_2\right), $$

$$ w_4 = w_3-\frac13\left(w_3^3-5+\eta_3\right). $$
...
$$ w_2 = 0-\left(0^3-5+0.2\right) = 4.8. $$

第 2 次：

$$ \begin{aligned} w_3 &= 4.8-\frac12\left(4.8^3-5-0.6\right)\\ &= 4.8-\frac12(110.592-5.6)\\ &= -47.696. \end{aligned} $$

第 3 次：

$$ \begin{aligned} w_4 &= -47.696-\frac13\left((-47.696)^3-5+0.4\right)\\ &\approx -47.696-\frac13(-108488.52-4.6)\\ &\approx 36116.68. \end{aligned} $$
...
$$ \eta_k\equiv 0, \qquad \tilde g(w_k,\eta_k)=g(w_k). $$
...
![原书图 6.4：单调函数下 RM 更新把估计推向根](/books/reinforcement-learning-math/images/b8e6a30b161441f62d35c5c40dbc6d004e666342c099b4dd73ac16be5a10e893.jpg)
> **原书图 6.4**：当 $w_k$ 在根 $w^*=1$ 右侧时，$g(w_k)\gt 0$，更新会往左移动；随着 $k$ 增大，估计逐渐靠近根。
...
$$ g(w)=0 \quad\Longleftrightarrow\quad w^*=2. $$
...
每一步都在根 $2$ 的右边，所以 $g(w_k)\gt 0$，更新不断把 $w_k$ 往左推。步长逐渐变小，所以后面会越来越稳。
...
$$ 0 \lt  c_{1} \leq \nabla_{w} g(w) \leq c_{2} \quad \text{for all } w. $$

意思是：

- $\nabla_w g(w)\ge c_1\gt 0$：$g$ 严格单调递增，所以根是唯一的；
...
$$ \sum_{k=1}^{\infty} a_k = \infty, \qquad \sum_{k=1}^{\infty} a_k^2 \lt  \infty. $$
...
$$ \sum_{k=1}^{\infty} a_k^2 \lt  \infty $$

强迫 $a_k\to 0$。直觉是：噪声一直存在，如果步长不变小，噪声会一直把 $w_k$ 推来推去，最后只能在根附近抖，难以真正收敛。

一句话记：

$$ \sum a_k=\infty \quad\text{保证走得够远；}\qquad \sum a_k^2\lt \infty \quad\text{保证噪声影响能被压下去。} $$
...
$$ \sum_{k=1}^{\infty}\frac1{k^2}=\frac{\pi^2}{6}\lt \infty. $$
...
$$ \mathbb E[\eta_k|\mathcal H_k]=0, \qquad \mathbb E[\eta_k^2|\mathcal H_k]\lt \infty. $$
...
$$ \underbrace{w_{k+1}-w_k}_{\text{第 }k\text{ 步位移}} = - \underbrace{a_k}_{\text{步长}} \underbrace{\tilde g(w_k,\eta_k)}_{\text{观测到的方向/大小}}. $$
...
$$ \sum_{k=1}^{\infty}|w_{k+1}-w_k| \le M\sum_{k=1}^{\infty}a_k. $$

现在关键来了：如果

$$ \sum_{k=1}^{\infty}a_k\lt \infty, $$
...
$$ \sum_{k=1}^{\infty}a_k = \frac12+\frac14+\frac18+\cdots =1. $$
...
$$ \begin{aligned} g(w)=0 &\Longleftrightarrow w-\mathbb E[X]=0\\ &\Longleftrightarrow w=\mathbb E[X]. \end{aligned} $$

所以“估计 $\mathbb E[X]$”和“求 $g(w)=0$ 的根”其实是同一件事：

$$ \text{estimate } \mathbb E[X] \quad\Longleftrightarrow\quad \text{find } w \text{ such that } g(w)=0. $$
...
$$ \mathbb E[\eta] = \mathbb E[\mathbb E[X]-x] = \mathbb E[X]-\mathbb E[x] =0. $$

于是 RM 更新变成：

$$ \begin{aligned} w_{k+1} &=w_k-\alpha_k\tilde g(w_k,\eta_k)\\ &=w_k-\alpha_k(w_k-x_k)\\ &=w_k+\alpha_k(x_k-w_k). \end{aligned} $$

这正好就是 6.1 的一般增量均值估计式。

因此，Robbins-Monro theorem 告诉我们：如果

$$ \sum_{k=1}^{\infty}\alpha_k=\infty, \qquad \sum_{k=1}^{\infty}\alpha_k^2\lt \infty, $$
...
$$ \Delta_k \doteq \text{当前估计}-\text{真实目标}. $$
...
$$ \Delta_{k+1} = (1-\alpha_k)\Delta_k+\beta_k\eta_k. \tag{6.7 前的形式} $$
...
$$ \text{新误差} = \underbrace{\text{旧误差被压小}}_{(1-\alpha_k)\Delta_k} + \underbrace{\text{新噪声影响}}_{\beta_k\eta_k}. $$

如果没有噪声，即 $\beta_k\eta_k=0$，那就是：

$$ \Delta_{k+1}=(1-\alpha_k)\Delta_k. $$

只要 $0\lt \alpha_k\lt 1$，每一步都会把误差压小一点。
...
$$ \sum_{k=1}^{\infty}\alpha_k=\infty, \qquad \sum_{k=1}^{\infty}\alpha_k^2\lt \infty, \qquad \sum_{k=1}^{\infty}\beta_k^2\lt \infty. $$

读法：

- $\sum\alpha_k=\infty$：压缩旧误差的动作要持续足够久；
- $\sum\alpha_k^2\lt \infty$：压缩步子最终要变小，避免一直震荡；
- $\sum\beta_k^2\lt \infty$：噪声的总体影响要有限。

**第二类：噪声条件**

$$ \mathbb E[\eta_k|\mathcal H_k]=0, \qquad \mathbb E[\eta_k^2|\mathcal H_k]\le C. $$
...
$$ \Delta_{k+1}-\Delta_k = -\alpha_k\Delta_k+\beta_k\eta_k, $$

$$ \Delta_{k+1}+\Delta_k = (2-\alpha_k)\Delta_k+\beta_k\eta_k. $$

于是：

$$ \begin{aligned} h_{k+1}-h_k &= \Delta_{k+1}^2-\Delta_k^2\\ &= (\Delta_{k+1}-\Delta_k)(\Delta_{k+1}+\Delta_k)\\ &= -\alpha_k(2-\alpha_k)\Delta_k^2\\ &\quad+\beta_k^2\eta_k^2\\ &\quad+2(1-\alpha_k)\beta_k\eta_k\Delta_k. \end{aligned} $$
...
$$ \begin{aligned} \mathbb E[h_{k+1}-h_k|\mathcal H_k] &= -\alpha_k(2-\alpha_k)\Delta_k^2\\ &\quad+\beta_k^2\mathbb E[\eta_k^2|\mathcal H_k]\\ &\quad+2(1-\alpha_k)\beta_k\Delta_k\mathbb E[\eta_k|\mathcal H_k]. \end{aligned} $$
...
另外，由 $\sum\alpha_k^2\lt \infty$ 可知 $\alpha_k\to0$，所以充分靠后时有 $\alpha_k\le1$。于是：

$$ -\alpha_k(2-\alpha_k)\Delta_k^2\le0. $$

因此得到原书的核心不等式：

$$ \mathbb E[h_{k+1}-h_k|\mathcal H_k] \le \beta_k^2 C. \tag{6.9} $$

由于：

$$ \sum_k\beta_k^2\lt \infty, $$
...
$$ \mathbb E[h_{k+1}-h_k|\mathcal H_k] = -\alpha_k(2-\alpha_k)\Delta_k^2 +\beta_k^2\mathbb E[\eta_k^2|\mathcal H_k]. $$

把负项移到左边：

$$ \alpha_k(2-\alpha_k)\Delta_k^2 = \beta_k^2\mathbb E[\eta_k^2|\mathcal H_k] - \mathbb E[h_{k+1}-h_k|\mathcal H_k]. $$

对 $k$ 从 1 到 $\infty$ 求和：

$$ \sum_{k=1}^{\infty}\alpha_k(2-\alpha_k)\Delta_k^2 = \sum_{k=1}^{\infty}\beta_k^2\mathbb E[\eta_k^2|\mathcal H_k] - \sum_{k=1}^{\infty}\mathbb E[h_{k+1}-h_k|\mathcal H_k]. $$
...
$$ \sum_{k=1}^{\infty}\beta_k^2\lt \infty. $$

所以：

$$ \sum_{k=1}^{\infty}\beta_k^2\mathbb E[\eta_k^2|\mathcal H_k] \le C\sum_{k=1}^{\infty}\beta_k^2 \lt \infty. $$
...
不会无限发散。因此右边第二项也是有界的。关于 $\mathbf 1_{A_k}$ 和“上涨部分”的直觉，见补充：[第 6 章补充：6.1-6.3 结构梳理](/books/reinforcement-learning-math/notes/06_supplement-6-1-6-3-structure/)。

因此左边也有界：

$$ \sum_{k=1}^{\infty}\alpha_k(2-\alpha_k)\Delta_k^2\lt \infty. $$
...
$$ \alpha_k(2-\alpha_k)\Delta_k^2 \ge \alpha_k\Delta_k^2. $$

因此可得：

$$ \sum_{k=1}^{\infty}\alpha_k\Delta_k^2\lt \infty. $$
...
如果 $\Delta_k^2$ 长期不接近 0，比如从某一刻开始一直大于某个 $\epsilon\gt 0$，那么：

$$ \sum_k\alpha_k\Delta_k^2 \ge \epsilon\sum_k\alpha_k $$

会发散。这和刚刚得到的

$$ \sum_k\alpha_k\Delta_k^2\lt \infty $$

矛盾。

所以只能是：

$$ \Delta_k^2\to0, \qquad \Delta_k\to0. $$
...
$$ w_{k+1}-w^* = w_k-w^*+\alpha_k(x_k-w_k). $$

关键是把括号拆成“噪声 - 误差”。对 $x_k-w_k$ 加减 $w^*$：

$$ \begin{aligned} x_k-w_k &=x_k-w^*+w^*-w_k\\ &=\underbrace{x_k-w^*}_{\eta_k} -\underbrace{(w_k-w^*)}_{\Delta_k}. \end{aligned} $$

代回去：

$$ \begin{aligned} \Delta_{k+1} &= \Delta_k+\alpha_k(\eta_k-\Delta_k)\\ &= (1-\alpha_k)\Delta_k+\alpha_k\eta_k. \end{aligned} $$
...
$$ \beta_k=\alpha_k, \qquad \eta_k=x_k-w^*. $$
...
$$ \mathbb E[\eta_k|\mathcal H_k] = \mathbb E[x_k-w^*|\mathcal H_k] =0. $$

如果 $x_k$ 方差有限，噪声二阶矩也有界。因此 Dvoretzky 定理给出：

$$ \Delta_k\to0, \qquad w_k\to w^*=\mathbb E[X]. $$
...
$$ \begin{aligned} \Delta_{k+1} &= \Delta_k-a_k[\nabla_w g(w_k')\Delta_k+\eta_k]\\ &= \left[1-a_k\nabla_w g(w_k')\right]\Delta_k+a_k(-\eta_k). \end{aligned} $$
...
$$ \alpha_k=a_k\nabla_wg(w_k'), \qquad \beta_k=a_k, \qquad \eta_k'=-\eta_k. $$
...
$$ \sum a_k=\infty, \qquad \sum a_k^2\lt \infty, $$

就能推出：

$$ \sum\alpha_k=\infty, \qquad \sum\alpha_k^2\lt \infty, \qquad \sum\beta_k^2\lt \infty. $$

再加上噪声条件均值为 0、二阶矩有限，Dvoretzky 定理条件全部满足。因此：

$$ \Delta_k\to0, \qquad w_k\to w^*. $$
...
$$ \Delta_{k+1}(s) = (1-\alpha_k(s))\Delta_k(s)+\beta_k(s)\eta_k(s), \qquad s\in S. $$
...
$$ \text{mean estimation} \subset \text{SGD} \subset \text{Robbins-Monro}. $$
...
$$ \mathbb E_{x_k}[\nabla_w f(w_k,x_k)] = \mathbb E_X[\nabla_w f(w_k,X)]. $$

因此可以写成：

$$ \nabla_w f(w_k,x_k) = \underbrace{\mathbb E[\nabla_w f(w_k,X)]}_{\text{真实梯度}} + \underbrace{\eta_k}_{\text{零均值噪声}}. $$
...
$$ \nabla_w f(w_k,x_k) = g(w_k)+\eta_k. $$

读法：单样本梯度 = 真实平均梯度 + 随机误差。

这个误差的条件均值为 0：

$$ \begin{aligned} \mathbb E[\eta_k|\mathcal H_k] &= \mathbb E[\nabla_w f(w_k,x_k)|\mathcal H_k] - g(w_k)\\ &= g(w_k)-g(w_k)\\ &= 0. \end{aligned} $$
...
$$ \min_w J(w) = \mathbb E\left[\frac12\|w-X\|^2\right]. \tag{6.14} $$
...
$$ J(w) = \mathbb E\left[\frac12(w-X)^2\right]. $$
...
读法：如果 $w\gt X$，梯度为正，梯度下降会把 $w$ 往小调；如果 $w\lt X$，梯度为负，梯度下降会把 $w$ 往大调。也就是说，每个样本都会把 $w$ 往自己这边拉。

真实目标函数的梯度是：

$$ \nabla_w J(w) = \nabla_w\mathbb E\left[\frac12\|w-X\|^2\right] = \mathbb E[\nabla_w f(w,X)] = \mathbb E[w-X] = w-\mathbb E[X]. $$
...
$$ \mathbb E[w-X] = \mathbb E[w]-\mathbb E[X] = w-\mathbb E[X]. $$

令它为 0：

$$ w-\mathbb E[X]=0 \quad\Longrightarrow\quad w^*=\mathbb E[X]. $$

所以这个优化问题和均值估计完全等价。

也可以用一个小例子看出来。假设样本只可能是：

$$ X\in\{2,4,8\}, \qquad p(X=2)=p(X=4)=p(X=8)=\frac13. $$
...
$$ J(w) = \frac13\left[ \frac12(w-2)^2 + \frac12(w-4)^2 + \frac12(w-8)^2 \right]. $$

求导：

$$ \begin{aligned} J'(w) &= \frac13[(w-2)+(w-4)+(w-8)]\\ &= w-\frac{14}{3}. \end{aligned} $$
...
$$ \nabla_wJ(w_k) = \mathbb E[w_k-X] = w_k-\mathbb E[X], $$
...
$$ \mathbb E[w_k-x_k|\mathcal H_k] = w_k-\mathbb E[X] = \nabla_wJ(w_k). $$
...
$$ \begin{aligned} w_{k+1} &= w_k-\alpha_k\nabla_w f(w_k,x_k)\\ &= w_k-\alpha_k(w_k-x_k)\\ &= w_k+\alpha_k(x_k-w_k). \end{aligned} $$

这正是 6.1 的增量均值估计式。

读法：每来一个新样本 $x_k$，就把当前估计 $w_k$ 朝 $x_k$ 的方向移动一点点，移动比例是 $\alpha_k$。

如果 $x_k\gt w_k$，那么 $x_k-w_k\gt 0$，更新会把 $w$ 往上推；如果 $x_k\lt w_k$，更新会把 $w$ 往下拉。长期来看，来自均值上方和下方的拉力会在 $\mathbb E[X]$ 附近平衡。

所以：

$$ \text{mean estimation update} = \text{SGD applied to } \frac12\|w-X\|^2. $$

这句话的含义是：6.1 看起来是在“算平均”，6.4 看起来是在“做优化”，但它们其实是同一个更新式的两种解释。

$$ \begin{array}{c} \text{均值估计视角：} w_{k+1}=w_k+\alpha_k(x_k-w_k)\\[4pt] \text{SGD 视角：} w_{k+1}=w_k-\alpha_k\nabla_w f(w_k,x_k) \end{array} $$
...
$$ \delta_k = \frac{|\mathbb E[X]-x_k|}{|w_k-w^*|}. $$

读法：越靠近真实均值 $w^*$，分母越小，随机样本 $x_k$ 带来的相对抖动越明显。

![原书图 6.5(a)：SGD 与 mini-batch GD 的二维轨迹](/books/reinforcement-learning-math/images/11994aa7585df8fd8f2e4a8c856caa05f516e6f56d509be35b69343b39f3c418.jpg)

![原书图 6.5(b)：不同 batch size 到均值的距离](/books/reinforcement-learning-math/images/f6c44d847aaa70e45f3496aeb6161721d874909ac35c70ecb5e9a6704ce575d4.jpg)
...
$$ \mathbb E[f(w,X)] = \frac1n\sum_{i=1}^n f(w,x_i). $$
...
$$ w_{k+1} = w_k-\alpha_k\frac1m\sum_{j\in\mathcal I_k}\nabla_w f(w_k,x_j). $$
...
$$ \bar x_k^{(m)} = \frac1m\sum_{j\in\mathcal I_k}x_j. $$
...
$$ \sum_k a_k=\infty, \qquad \sum_k a_k^2\lt \infty, $$
...
$$ \underbrace{\text{mean estimation 均值估计}}_{\text{6.1, 式(6.4)}} \;\subset\; \underbrace{\text{SGD 随机梯度下降}}_{\text{6.4}} \;\subset\; \underbrace{\text{Robbins-Monro 随机近似}}_{\text{6.2}} $$
...
| 什么是 MBGD？强在哪？ | mini-batch GD 是 SGD 与 BGD 的**折中**：比 SGD 随机性小（用多样本），比 BGD 灵活（不用全部样本） | 样本数 $1\lt m\lt n$ 的中间档 |
...
$$ \text{6.1 mean estimation} \rightarrow \text{6.2 Robbins-Monro} \rightarrow \text{6.3 convergence proof} $$

这条线是随机近似主线：有噪声、用步长、证明会收敛。

而 6.4 是把另一条熟悉的优化线接进来：

$$ \text{optimization} \rightarrow \text{gradient descent} \rightarrow \text{SGD} $$

然后作者要说明：这条优化线其实也能放回 RM 框架里。

所以 6.4 的真正位置是：

$$ \text{SGD} \subset \text{Robbins-Monro stochastic approximation}. $$
...
$$ \min_w J(w) \quad\Longrightarrow\quad \nabla_w J(w)=0. $$
