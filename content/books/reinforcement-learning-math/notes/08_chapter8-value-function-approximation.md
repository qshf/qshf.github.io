---
title: "第 8 章 价值函数近似（Value Function Approximation）- 8.1-8.6"
...
$$ \underbrace{\hat v(s,w)}_{\text{对 }v_\pi(s)\text{ 的估计}} = \underbrace{\phi^T(s)}_{\text{由状态 }s\text{ 产生的特征行向量}} \underbrace{w}_{\text{要学习的参数向量}}. $$

读法：先把状态 $s$ 变成 feature vector 特征向量 $\phi(s)$，再和参数 $w$ 做内积，得到该状态的价值估计。

这里：

$$ \phi(s)= \begin{bmatrix} s\\ 1 \end{bmatrix}, \qquad w= \begin{bmatrix} a\\ b \end{bmatrix}. $$

$\hat v(s,w)$ 也常写成 $\hat v_w(s)$。这两个写法含义一样，只是前者更明确地把参数 $w$ 写出来。

![原书图 8.3：函数近似中读取状态价值的过程](/books/reinforcement-learning-math/images/67cf40a3b6441de8866d6e739abb2390da6bb709308722aa35c2d058c310bbab.jpg)
...
![原书图 8.4(a)：表格方法更新一个状态只改变该状态](/books/reinforcement-learning-math/images/5ac3ada5395a3e0bbff5ceef038662a4cbf2d4136fbad622c6f0203534b2a385.jpg)
...
![原书图 8.4(b)：函数近似更新参数会同时影响邻近状态](/books/reinforcement-learning-math/images/5dad00e1a42623ba361bad2efc76f3870b211379d1963c890a3e7e65b1b38dd7.jpg)
...
$$ \phi(s)= \begin{bmatrix} s^2\\ s\\ 1 \end{bmatrix}, $$
...
$$ \hat v(s,w)= a\cdot s^2+b\cdot s+c\cdot 1. $$
...
$$ \Phi \doteq \left[ \begin{array}{c} \phi^ {T} (s _ {1}) \\ \vdots \\ \phi^ {T} (s _ {n}) \end{array} \right], \qquad v _ {\pi} \doteq \left[ \begin{array}{c} v _ {\pi} (s _ {1}) \\ \vdots \\ v _ {\pi} (s _ {n}) \end{array} \right]. $$
...
$$ \underbrace{\Phi}_{n\times m} \underbrace{w}_{m\times 1} - \underbrace{v_\pi}_{n\times 1} = \underbrace{\Phi w-v_\pi}_{n\times 1}. $$
...
$$ \underbrace{\Phi^T}_{m\times n} \underbrace{\Phi}_{n\times m} = \underbrace{\Phi^T\Phi}_{m\times m}, \qquad \underbrace{(\Phi^T\Phi)^{-1}}_{m\times m}, $$

以及：

$$ \underbrace{\Phi^T}_{m\times n} \underbrace{v_\pi}_{n\times 1} = \underbrace{\Phi^T v_\pi}_{m\times 1}. $$

最后：

$$ \underbrace{(\Phi^T\Phi)^{-1}}_{m\times m} \underbrace{\Phi^T v_\pi}_{m\times 1} = \underbrace{w^*}_{m\times 1}. $$
...
$$ \text{表格价值 } \hat v(s_i) \quad\Longrightarrow\quad \text{函数价值 } \hat v(s,w). $$
...
$$ \text{定义误差目标 }J(w) \quad\Longrightarrow\quad \text{用梯度下降更新 }w \quad\Longrightarrow\quad \text{真实 }v_\pi\text{ 未知，换成 MC 或 TD 目标} \quad\Longrightarrow\quad \text{得到基于函数近似的 TD 更新}. $$
...
$$ \underbrace{d_\pi(s)}_{\text{长期访问 }s\text{ 的概率}} \underbrace{(v_\pi(s)-\hat v(s,w))^2}_{\text{状态 }s\text{ 的平方误差}}. $$
...
$$ \lim  _ {k \rightarrow \infty} d _ {k} ^ {T} = d _ {\pi} ^ {T}. \tag {8.9} $$
...
$$ P_\pi= \begin{bmatrix} 0.8 & 0.2\\ 0.4 & 0.6 \end{bmatrix}. $$
...
$$ d_1^T=d_0^TP_\pi=[1,0] \begin{bmatrix} 0.8 & 0.2\\ 0.4 & 0.6 \end{bmatrix} =[0.8,0.2]. $$

再走一步：

$$ d_2^T=d_1^TP_\pi=[0.8,0.2] \begin{bmatrix} 0.8 & 0.2\\ 0.4 & 0.6 \end{bmatrix} =[0.72,0.28]. $$
...
$$ [d_1,d_2] = [d_1,d_2] \begin{bmatrix} 0.8 & 0.2\\ 0.4 & 0.6 \end{bmatrix}, \qquad d_1+d_2=1. $$

第一维给：

$$ d_1=0.8d_1+0.4d_2 \Longrightarrow 0.2d_1=0.4d_2 \Longrightarrow d_1=2d_2. $$
...
$$ d_\pi= \begin{bmatrix} 2/3\\ 1/3 \end{bmatrix}. $$
...
![原书图 8.5 左：四状态网格中的策略与转移结构](/books/reinforcement-learning-math/images/7b802eceba4899c6241fe6a1636c3f53fbb6f94ad0fcfb925667919956e517d3.jpg)
![原书图 8.5 右：经验访问比例收敛到理论平稳分布](/books/reinforcement-learning-math/images/325975969d05010c064359b2d55371f7c1338f2027e2347a03fff48863c49eb6.jpg)
...
$$ w _ {t + 1} = w _ {t} + \alpha_ {t} \left[ \underbrace{r _ {t + 1} + \gamma \hat {v} \left(s _ {t + 1}, w _ {t}\right)}_{\text{TD target}} - \underbrace{\hat {v} \left(s _ {t}, w _ {t}\right)}_{\text{当前估计}} \right] \nabla_ {w} \hat {v} \left(s _ {t}, w _ {t}\right). \tag {8.13} $$
...
$$ \hat v(s,w)=\phi^T(s)w,\qquad \phi(s_t)= \begin{bmatrix} 1\\ 2 \end{bmatrix}, \quad \phi(s_{t+1})= \begin{bmatrix} 1\\ 3 \end{bmatrix}, \quad w_t= \begin{bmatrix} 0.5\\ 0.2 \end{bmatrix}. $$

取 $r_{t+1}=1$，$\gamma=0.9$，$\alpha_t=0.1$。

先算当前状态估计：

$$ \hat v(s_t,w_t)=[1,2] \begin{bmatrix} 0.5\\ 0.2 \end{bmatrix} =0.9. $$

再算下个状态估计：

$$ \hat v(s_{t+1},w_t)=[1,3] \begin{bmatrix} 0.5\\ 0.2 \end{bmatrix} =1.1. $$
...
$$ w_{t+1} = \begin{bmatrix} 0.5\\ 0.2 \end{bmatrix} +0.1\times 1.09 \begin{bmatrix} 1\\ 2 \end{bmatrix} = \begin{bmatrix} 0.609\\ 0.418 \end{bmatrix}. $$
...
$$ w _ {t + 1} = w _ {t} + \alpha_ {t} \left[ r _ {t + 1} + \gamma \hat {v} (s _ {t + 1}, w _ {t}) - \hat {v} (s _ {t}, w _ {t}) \right] \nabla_ {w}\hat {v} (s _ {t}, w _ {t}). $$
...
![原书图 8.6(a)：待评估策略](/books/reinforcement-learning-math/images/4b446653e6cfa96d1cc1f23b444c79fbb8cc7d270fcaa1163485399a09bb07d7.jpg)
![原书图 8.6(b)：真实状态值表](/books/reinforcement-learning-math/images/07319ba5cc743e15abf83e7fb07cce4d9fd7e30c198052040d8ab92dd95ddef9.jpg)
![原书图 8.6(c)：真实状态值的三维曲面](/books/reinforcement-learning-math/images/d1b7ac7391824bf157aec21c857c7f042e5fa10622d13ab065a7991e70455658.jpg)
...
$$ \hat v(s,w) = w_1+w_2x+w_3y+w_4x^2+w_5y^2+w_6xy. $$

再增强到三次相关特征：

$$ \phi (s) = \left[ 1, x, y, x ^ {2}, y ^ {2}, x y, x ^ {3}, y ^ {3}, x ^ {2} y, x y ^ {2} \right] ^ {T} \in \mathbb {R} ^ {10}. \tag {8.17} $$
...
![原书图 8.7(a1)：三维多项式特征，估计价值曲面](/books/reinforcement-learning-math/images/90c05dc99c13d8312194932b309d244f548c681cf4b7f778ed7181e23a8f9a4f.jpg)
![原书图 8.7(a2)：三维多项式特征，RMSE 曲线](/books/reinforcement-learning-math/images/c3bfa3c5ab6dbe42599ae1798eb44a48e614863a1b70bd36dd5656488b51a827.jpg)
![原书图 8.7(b1)：六维多项式特征，估计价值曲面](/books/reinforcement-learning-math/images/e5d3066f842215769addfedfb1be2671cf43e130ee87570e45c70af36788db68.jpg)
![原书图 8.7(b2)：六维多项式特征，RMSE 曲线](/books/reinforcement-learning-math/images/76b147bb93f5c0244ade2db98afa2b6b72e1c9613df9b6888f26a7ecec010fb0.jpg)
![原书图 8.7(c1)：十维多项式特征，估计价值曲面](/books/reinforcement-learning-math/images/e88253e1ee3fee4c680e8710e8386758dc2c0a0339779ec504831fde0584a4fb.jpg)
![原书图 8.7(c2)：十维多项式特征，RMSE 曲线](/books/reinforcement-learning-math/images/c326e3fcfd10bfb2acb745b140d9a13f043086578717d481dd058969ac9f2c28.jpg)
...
$$ \phi(s)= \left[ \begin{array}{c} 1\\ \cos(\pi y)\\ \cos(\pi x)\\ \cos(\pi(x+y)) \end{array} \right] \in\mathbb R^4. $$
...
![原书图 8.8(a1)：Fourier 特征，q=1，估计价值曲面](/books/reinforcement-learning-math/images/cb815f2c2d47d001e2898e6e0fbb8b030a2f0e5c35fd396957efd3f7af496d7f.jpg)
![原书图 8.8(a2)：Fourier 特征，q=1，RMSE 曲线](/books/reinforcement-learning-math/images/fa87f2828830c2f20fd90903f040cda4031e5c0398e287755b7f61717c234325.jpg)
![原书图 8.8(b1)：Fourier 特征，q=2，估计价值曲面](/books/reinforcement-learning-math/images/40821099fb6555439b9eae6856922fc3c5e03a827fa9e4c706010cbd4df99dd1.jpg)
![原书图 8.8(b2)：Fourier 特征，q=2，RMSE 曲线](/books/reinforcement-learning-math/images/88b9ccd2893261176d10bcd1be907f917ded6827405bfd42434d9046ee8ab588.jpg)
![原书图 8.8(c1)：Fourier 特征，q=3，估计价值曲面](/books/reinforcement-learning-math/images/b8a0026b0f0385990b39018d9a807ebe38846c6d92283f262dfb4b4ddda9f1c0.jpg)
![原书图 8.8(c2)：Fourier 特征，q=3，RMSE 曲线](/books/reinforcement-learning-math/images/07aa07ded7d990d4b5b1d096447d87aa319da35bb5c0198e2534e032869b5a60.jpg)
...
$$ \Phi = \left[ \begin{array}{c} \vdots \\ \phi^ {T} (s) \\ \vdots \end{array} \right] \in \mathbb {R} ^ {n \times m}, \qquad D = \left[ \begin{array}{c c c} \ddots & & \\ & d _ {\pi} (s) & \\ & & \ddots \end{array} \right] \in \mathbb {R} ^ {n \times n}. \tag {8.20} $$
...
这一推导的详细版单独整理在：[补充：Lemma 8.1 中 b-Aw_t 的详细推导](/books/reinforcement-learning-math/notes/08_supplement-lemma-8-1-b-minus-aw-derivation/)。
...
$$ \mathbb {E} \left[ \left( r _ {t + 1} + \gamma \phi^ {T} (s _ {t + 1}) w _ {t} - \phi^ {T} (s _ {t}) w _ {t} \right) \phi (s _ {t}) \right]. $$

因为 $s_t$ 按平稳分布 $d_\pi$ 出现，所以可以先按当前状态 $s_t=s$ 分解：

$$ \sum_{s\in\mathcal S} d_\pi(s)\; \mathbb E\!\left[ \left( r_{t+1} + \gamma \phi^T(s_{t+1})w_t - \phi^T(s)w_t \right)\phi(s_t) \mid s_t=s \right]. $$

然后把它拆成两部分：

$$ \underbrace{ \mathbb E[r_{t+1}\phi(s_t)\mid s_t=s] }_{\text{reward 项}} \;+\; \underbrace{ \mathbb E\!\left[\phi(s_t)\big(\gamma\phi^T(s_{t+1})-\phi^T(s_t)\big)w_t\mid s_t=s\right] }_{\text{TD 项}}. $$

reward 项很直接：

$$ \mathbb E[r_{t+1}\phi(s_t)\mid s_t=s] = \phi(s)\,r_\pi(s), $$

因此加总后得到：

$$ \sum_{s\in\mathcal S} d_\pi(s)\phi(s)r_\pi(s) = \Phi^TDr_\pi =b. $$

TD 项则整理成：

$$ -\Phi^T D\Phi w_t +\gamma \Phi^T D P_\pi \Phi w_t = -\Phi^T D(I-\gamma P_\pi)\Phi w_t = -Aw_t. $$
...
$$ A=D(I-\gamma P_\pi), \qquad b=Dr_\pi. $$

所以：

$$ w^* =A^{-1}b = (I-\gamma P_\pi)^{-1}D^{-1}Dr_\pi = (I-\gamma P_\pi)^{-1}r_\pi =v_\pi. \tag {8.23} $$
...
$$ \text{TD-Linear 不直接最小化 }J_E(w) =\mathbb E[(v_\pi(S)-\hat v(S,w))^2]. $$
...
$$ J _ {E} (w) = \mathbb {E} [ (v _ {\pi} (S) - \hat {v} (S, w)) ^ {2} ] = \| \hat {v} (w) - v _ {\pi} \| _ {D} ^ {2}. $$
...
$$ J_{BE}(w) = \|\hat v(w)-T_\pi(\hat v(w))\|_D^2. $$
...
$$ T_\pi(\Phi w) \quad\longrightarrow\quad M T_\pi(\Phi w). $$

这时 projected Bellman error 要比较的两边就变成：

$$ \underbrace{\Phi w}_{\text{当前估计，已经在特征空间里}} - \underbrace{M T_\pi(\Phi w)}_{\text{backup 后投影回特征空间}}. $$
...
$$ \Phi w = M T_\pi(\Phi w) = \Phi(\Phi^T D\Phi)^{-1}\Phi^T D\bigl(r_\pi+\gamma P_\pi \Phi w\bigr). $$

因为我们要找的是同一个特征空间里的向量，所以左边和右边都在 $\Phi$ 的列空间里。把左边也写成 $\Phi$ 乘某个参数向量，就可以比较参数：

$$ w = (\Phi^T D\Phi)^{-1}\Phi^T D\bigl(r_\pi+\gamma P_\pi \Phi w\bigr). $$

两边左乘 $\Phi^T D\Phi$，得到

$$ \Phi^T D\Phi w = \Phi^T D r_\pi +\gamma \Phi^T D P_\pi \Phi w. $$

移项：

$$ \Phi^T D(I-\gamma P_\pi)\Phi w = \Phi^T D r_\pi. $$
...
$$ b-Aw=0 \quad\Longleftrightarrow\quad Aw=b. $$
...
$$ \text{TD 更新停住} \quad\Longleftrightarrow\quad Aw=b \quad\Longleftrightarrow\quad \text{投影 Bellman 固定点成立}. $$
...
$$ \| \Phi w ^ {*} - v _ {\pi} \| _ {D} \leq \frac {1}{1 - \gamma} \min _ {w} \| \hat {v} (w) - v _ {\pi} \| _ {D}. \tag {8.32} $$
...
$$ \hat v(s) \quad\Longrightarrow\quad \hat v(s,w) \quad\Longrightarrow\quad w_{t+1}=w_t+\alpha_t\delta_t\nabla_w\hat v(s_t,w_t). $$
...
![原书图 8.9(a)：Sarsa with linear function approximation 的总回报](/books/reinforcement-learning-math/images/27794bd9481d0247c36a7146203df9a0ed583b933ab3f90045c50136697348c5.jpg)
> **原书图 8.9(a)**：总回报随 episode index 增大后逐渐稳定，说明策略在变好。

![原书图 8.9(b)：Sarsa with linear function approximation 的 episode length](/books/reinforcement-learning-math/images/bfc78edb60f1255e84615df748df6dc0841aec0e5404a5f0819aa69d40114ded.jpg)
> **原书图 8.9(b)**：episode length 很快下降并稳定，说明到达目标所需步数越来越少。

![原书图 8.9(c)：Sarsa 学到的路径策略](/books/reinforcement-learning-math/images/9581a3713f79dea223403cd7b3b74903cd0247eaf5d8d612e0067d08c3f3b130.jpg)
...
![原书图 8.10(a)：Q-learning with linear function approximation 的总回报](/books/reinforcement-learning-math/images/3f18eb155985ac38e5eea1607670c6f2099d75203f8032bc8e3f7d9ca4b9e41b.jpg)
> **原书图 8.10(a)**：总回报在最初剧烈波动后迅速稳定。

![原书图 8.10(b)：Q-learning with linear function approximation 的 episode length](/books/reinforcement-learning-math/images/9328e6d11df3a644ec901debe1478c8fd9bf6cd1631630e930e121dd23bdc668.jpg)
> **原书图 8.10(b)**：episode length 很快压到接近 0 的水平，说明路径越来越短。

![原书图 8.10(c)：Q-learning 学到的路径策略](/books/reinforcement-learning-math/images/170aa0da1e6560c5e268828af1181b3048337816e199a919565a6381a1ad8793.jpg)
...
$$ \text{TD target} \quad\Longrightarrow\quad \text{TD error} \quad\Longrightarrow\quad \text{参数沿梯度方向更新} $$
...
$$ w_{t+1} = w_t + \alpha_t \Big[ r_{t+1} +\gamma \max_{a}\hat q(s_{t+1},a,w_t) -\hat q(s_t,a_t,w_t) \Big] \nabla_w\hat q(s_t,a_t,w_t). $$
...
$$ \underbrace{R+\gamma\max_a\hat q(S',a,w)}_{\text{Bellman optimality target}} - \underbrace{\hat q(S,A,w)}_{\text{当前网络输出}} $$
...
$$ (s_0,a_0,r_1,s_1), (s_1,a_1,r_2,s_2), (s_2,a_2,r_3,s_3), \ldots $$
...
$$ y_T \in \mathbb R^B, \qquad \hat y=\hat q(s,a,w)\in \mathbb R^B. $$
...
$$ y_T^{(i)} = r^{(i)} +\gamma\max_a\hat q(s'^{(i)},a,w_T), \qquad i=1,2,3. $$

然后最小化 batch loss：

$$ L(w) = \frac{1}{3} \sum_{i=1}^{3} \left( y_T^{(i)} -\hat q(s^{(i)},a^{(i)},w) \right)^2. $$
...
$$ \pi_b\text{ 采样} \rightarrow \mathcal B\text{ 存储} \rightarrow \text{随机 mini-batch} \rightarrow w_T\text{ 算 target} \rightarrow w\text{ 更新主网络} \rightarrow \text{周期性 }w_T=w. $$
...
![原书图 8.11(a)：用于收集经验的探索性行为策略](/books/reinforcement-learning-math/images/4d23b9e6a8051c042f36073d52b4998ba227c30c50c7941141b9ead97808fdcd.jpg)
> **原书图 8.11(a)**：行为策略在每个状态对各动作给出相同概率，所以探索性很强。

![原书图 8.11(b)：1000 步 episode 访问了几乎所有状态-动作对](/books/reinforcement-learning-math/images/0f5f8cdeae33733ef6deb34066dbdbeaa7a937debbc58b62e130acdaee95f701.jpg)
> **原书图 8.11(b)**：虽然只有 1000 步，但因为行为策略探索充分，几乎所有状态-动作对都被访问到了。

![原书图 8.11(c)：DQN 学到的最终贪心策略](/books/reinforcement-learning-math/images/f500755199ce2a08d0003a9f78f9f541aa51d1d8ba007ce4e6ad8e5dc51642c9.jpg)
> **原书图 8.11(c)**：学到的最终策略能绕开障碍，到达目标状态。

![原书图 8.11(d)：mini-batch 平均平方 TD 误差收敛到 0](/books/reinforcement-learning-math/images/ff57cc6e03432c7c2ccd55c22c5818fcf3fffc4ab6680d10b47aaa35e9c5ca75.jpg)
> **原书图 8.11(d)**：loss function 损失函数逐渐降到接近 0，说明网络很好地拟合了 replay buffer 里的训练样本。

![原书图 8.11(e)：状态值估计误差收敛到 0](/books/reinforcement-learning-math/images/6033789a8c8b1745cae74378d19fcd164fdaaa1058aa29033e72a99cfe8e02f4.jpg)
...
$$ s\longmapsto \big[ \hat q(s,a_1),\ldots,\hat q(s,a_5) \big]. $$
...
![原书图 8.12(a)：同样的探索性行为策略](/books/reinforcement-learning-math/images/e783226066dd09bff0619cbc1c6c3608bc72c04568b190785fc079f44ac53840.jpg)
> **原书图 8.12(a)**：行为策略仍然是强探索策略。

![原书图 8.12(b)：只有 100 步的 episode](/books/reinforcement-learning-math/images/580ec29904d36cdf7de152eb6b2a4dbacdd4971e053f1013e9ae535171fbcd33.jpg)
> **原书图 8.12(b)**：经验覆盖明显不足，很多状态-动作对没有被充分访问。

![原书图 8.12(c)：经验不足时学到的最终策略](/books/reinforcement-learning-math/images/a06741080c705eab2c9524f9ff281506d86c77ddf030712e0f3c6759de6ebc71.jpg)
> **原书图 8.12(c)**：最终策略不再可靠，很多位置的动作选择明显不如图 8.11。

![原书图 8.12(d)：训练 loss 仍然能收敛到 0](/books/reinforcement-learning-math/images/c5bddd80b54b736b5b37155396c61464edd2a52a1052d6fe956a7e37e4c8cfa6.jpg)
> **原书图 8.12(d)**：网络可以把已有 replay buffer 样本拟合得很好。

![原书图 8.12(e)：但真实 value error 没有收敛到 0](/books/reinforcement-learning-math/images/174182f424ea0c9e0172b6ae87f3e704455bad60b6f311f657b8e572f9896418.jpg)
...
$$ L_{\text{DQN}}(w) = \frac{1}{B}\sum_{i=1}^{B} \left( y_T^{(i)} -\hat q(s^{(i)},a^{(i)},w) \right)^2 $$

$$ E_{\text{value}}(w) = \sum_{s,a} \left( \hat q(s,a,w)-q^*(s,a) \right)^2 $$

两者的关系是：

$$ L_{\text{DQN}} = \text{replay buffer 上的经验 Bellman/TD loss} \neq \text{真实 value error} $$

$$ E_{\text{value}} = \text{相对 }q^*\text{ 的真实误差} $$

所以，上面的 $L_{\text{DQN}}$ **不是真实误差**。它是训练时用 mini-batch 能算出来的代理目标，因为 $y_T^{(i)}$ 可以由 target network 算出：

$$ y_T^{(i)} = r^{(i)} +\gamma\max_a\hat q(s'^{(i)},a,w_T). $$
...
$$ L_{\text{DQN}}(w)\approx 0 \not\Longrightarrow E_{\text{value}}(w)\approx 0 $$

$$ \mathcal B\ \text{coverage small} \quad\Longrightarrow\quad L_{\text{DQN}}(w)\downarrow \ \text{but}\ E_{\text{value}}(w)\nrightarrow 0 $$
...
$$ \text{Q-learning target} \quad\Longrightarrow\quad \text{target network 固定目标} \quad\Longrightarrow\quad \text{experience replay 打散样本} \quad\Longrightarrow\quad \text{mini-batch 神经网络训练} $$
...
$$ s \rightarrow \text{network}(w) \rightarrow \hat v(s,w). $$
...
$$ w\in\mathbb R^m, \qquad m\ll |\mathcal S|\ \text{通常成立}. $$
...
$$ \phi(s_i)=e_i, \qquad \hat v(s_i,w)=\phi(s_i)^T w=e_i^T w=w_i. $$

这说明：表格里的第 $i$ 个格子，本质上就是参数向量 $w$ 的第 $i$ 个分量。

所以：

$$ \text{tabular value} \equiv \text{linear function approximation with one-hot features}. $$
...
$$ d_\pi = \begin{bmatrix} d_\pi(s_1)&d_\pi(s_2)&\cdots&d_\pi(s_n) \end{bmatrix}^T. $$

它在本章出现，是因为价值函数近似需要定义 objective function 目标函数。比如加权平方误差：

$$ J(w) = \sum_s d_\pi(s) \left( v_\pi(s)-\hat v(s,w) \right)^2. $$
...
$$ J = \mathbb E \left[ \left( R+\gamma\max_a\hat q(S',a,w)-\hat q(S,A,w) \right)^2 \right]. \tag{8.37} $$
...
$$ \text{sequence samples} \rightarrow \mathcal B \rightarrow \text{uniform mini-batch}. $$
...
$$ \underbrace{\hat q(S,A,w)}_{\text{当前输出}} \quad\text{和}\quad \underbrace{R+\gamma\max_a\hat q(S',a,w)}_{\text{target}}. $$
...
$$ \mathcal B_{\text{mini}} \rightarrow y_T \rightarrow L(w) = \frac1B\sum_i \left( y_T^{(i)}-\hat q(s^{(i)},a^{(i)},w) \right)^2 \rightarrow \text{optimizer step}. $$
...
2. 为什么动作值部分没有展开 projected Bellman error？是因为非线性吗？
   ...
   $$ \text{fixed policy evaluation} + \text{linear function approximation} $$

   也就是给定策略 $\pi$，学习 $v_\pi$ 或 $q_\pi$ 的理论分析。

   如果是固定策略的动作值评估，也可以写类似的投影 Bellman 方程：

   $$ \hat q(s,a,w)=\phi(s,a)^T w, \qquad \hat q(w)=\Phi_Qw. $$

   $$ \Phi_Q w = M_QT_\pi^Q(\Phi_Q w). $$
