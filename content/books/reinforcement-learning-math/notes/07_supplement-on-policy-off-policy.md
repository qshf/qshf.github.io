---
title: "第 7 章补充：on-policy 与 off-policy 怎么理解"
...
$$ \boxed{ \text{behavior policy}=\text{target policy} \quad\Longrightarrow\quad \text{on-policy} } $$

$$ \boxed{ \text{behavior policy}\neq\text{target policy}\text{ 也可以学习} \quad\Longrightarrow\quad \text{off-policy} } $$
...
$$ s_t \xrightarrow[\pi_t]{\text{采样}} a_t \to r_{t+1},s_{t+1} \xrightarrow[\pi_t\text{ 或更新后的 }\pi_{t+1}]{\text{采样}} a_{t+1}. $$
...
$$ r_{t+1} + \gamma q_t(B,\text{Left}) = r_{t+1} + 2\gamma. $$
...
$$ \pi_b=\epsilon\text{-greedy} \quad\text{or}\quad \pi_b=\text{uniform}. $$
...
$$ r_{t+1} + \gamma\max\{2,8\} = r_{t+1} + 8\gamma. $$
...
$$ \text{采样用的策略} \quad \text{和} \quad \text{正在学习的策略} \quad \text{是同一个。} $$

Sarsa 中策略可以随 $q_t$ 变化：

$$ q_t \to \pi_t=\epsilon\text{-greedy}(q_t) \to \text{采样} \to q_{t+1} \to \pi_{t+1}=\epsilon\text{-greedy}(q_{t+1}). $$
...
$$ \pi_b=\text{uniform 或 }\epsilon\text{-greedy}, \qquad \pi_T=\text{greedy}. $$
...
$$ \boxed{ \text{用实际走出来的策略学它自己：on-policy} } $$

$$ \boxed{ \text{用一套策略采样，学另一套策略：off-policy} } $$
