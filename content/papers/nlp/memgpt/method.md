...
![MemGPT 在上下文不足时写入持久记忆（Memory Creation）](/images/memgpt/memgpt_diagrams_wide_memory_creation.png)

*图（对应论文 `example-memory-creation`）：MemGPT 收到上下文空间不足的系统告警后，自发把重要信息写入持久化记忆。*

![MemGPT 系统总体流程（System Flow）](/images/memgpt/memgpt_system_flow_2.png)
...
![MemGPT 在对话中更新已存储信息（写入 working context）](/images/memgpt/memgpt_diagrams_wide_memory_correction.png)
